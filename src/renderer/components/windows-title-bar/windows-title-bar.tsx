...
export default class WindowsTitleBar extends React.Component<unknown, IState> {
  ...
  constructor(props: {}) {
    ...
    window.electron.ipcRenderer.on(TitleBarEvents.MAXIMIZE, () => this.updateState({ isMaximized: true }));
    window.electron.ipcRenderer.on(TitleBarEvents.UNMAXIMIZE, () =>
      this.updateState({ isMaximized: false }),
    );
    window.electron.ipcRenderer.on(TitleBarEvents.MOVE, (_event, isMaximized) => {
      this.updateState({ isMaximized });
    });

    window.electron.ipcRenderer.once(TitleBarEvents.DISABLE_ACTION_BUTTON, () => {
