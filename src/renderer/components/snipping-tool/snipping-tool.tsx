...
import { ReactComponent as PenIcon } from '../../assets/snip-draw.svg';
import { ReactComponent as EraseIcon } from '../../assets/snip-erase.svg';
import { ReactComponent as HighlightIcon } from '../../assets/snip-highlight.svg';

// import AnnotateArea from './annotate-area';
import ColorPickerPill, { IColor } from './color-picker-pill';
import MenuButton from './menu-button';

import './snipping-tool.less';
import AnnotateArea from './annotate-area';
import { AnalyticsElements, ScreenSnippetActionTypes } from 'app/bi/interface';
...
export const sendAnalyticsToMain = (
  ...
  window.electron.ipcRenderer.sendMessage(ScreenShotAnnotationEvents.ANALYTICS, {
    element,
    type,
  });
};

export default function SnippingTool({ existingPaths }: ISnippingToolProps) {
  ...
  const onCopyToClipboard = async (eventName: ScreenShotAnnotationEvents.COPY_TO_CLIPBOARD) => {
    const img = await mergeImage();
    sendAnalyticsToMain(
      AnalyticsElements.SCREEN_CAPTURE_ANNOTATE,
      ScreenSnippetActionTypes.ANNOTATE_COPY
    );
    window.electron.ipcRenderer.sendMessage(eventName, {clipboard: img})
  };

  const onSaveAs = async (eventName: ScreenShotAnnotationEvents.SAVE_AS) => {
    const img = await mergeImage();
    sendAnalyticsToMain(
      AnalyticsElements.SCREEN_CAPTURE_ANNOTATE,
  ...
  const menuItem = [
    {
      ...
      event: ScreenShotAnnotationEvents.COPY_TO_CLIPBOARD,
      onClick: onCopyToClipboard,
      dataTestId: 'COPY_TO_CLIPBOARD',
    },
    {
      name: i18n.t('Save as', SNIPPING_TOOL_NAMESPACE)(),
      event: ScreenShotAnnotationEvents.SAVE_AS,
      onClick: onSaveAs,
      dataTestId: 'SAVE_AS',
    },
  ];
  ...
  useLayoutEffect(() => {
    window.electron.ipcRenderer.once(ScreenShotAnnotationEvents.DATA, (args) => {
      const {
            snipImage,
            annotateAreaHeight,
            annotateAreaWidth,
  ...
  const addToChat = async () => {
    ...
    window.electron.ipcRenderer.sendMessage(ScreenShotAnnotationEvents.UPLOAD_SNIPPET, { screenSnippetPath, mergedImageData });
  };
  ...
  const onClose = async () => {
    ...
    window.electron.ipcRenderer.sendMessage(ScreenShotAnnotationEvents.CLOSE);
