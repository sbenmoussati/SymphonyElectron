...
class Notification extends NotificationHandler {
  ...
  constructor(opts: unknown) {
    super(opts);
    ipcMain.on(NotificationEvents.CLOSE, (_event, windowId) => {
    ...
    ipcMain.on(NotificationEvents.CLICKED, (_event, windowId) => {
      this.notificationClicked(windowId);
    });
    ipcMain.on(NotificationEvents.MOUSE_ENTER, this.funcHandlers.onMouseOver);
    ipcMain.on(NotificationEvents.MOUSE_LEAVE, this.funcHandlers.onMouseLeave);
    ipcMain.on(NotificationEvents.ON_REPLY, (_event, windowId, replyText) => {
      this.onNotificationReply(windowId, replyText);
    });
    ipcMain.on(NotificationEvents.ON_IGNORE, (_event, windowId) => {
      this.onNotificationIgnore(windowId);
    });
    ipcMain.on(NotificationEvents.SHOW_REPLY, this.funcHandlers.onShowReply);
   ...
   * @param clientId {number}
   */
  public notificationClosed(clientId: number): void {
    const browserWindow = this.getNotificationWindow(clientId);
    if (
      browserWindow &&
      windowExists(browserWindow) &&
